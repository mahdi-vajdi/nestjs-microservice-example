...
export class JwtTokenGenerator implements TokenGeneratorPort {
  ...
  async generateAccessToken(userId: string, role: string): Promise<{ token: string; expiresIn: number }> {
  ...
  async verifyAccessToken(token: string): Promise<{ userId: string; role: string; isValid: boolean }> {
    try {
